<template>
  <ElCard>
    <template #header> 文件下载功能 </template>

    <ElButton @click="handleDownloadOnlineImg" type="primary">下载在线图片</ElButton>
    <ElButton @click="handleDownloadBase64Img" type="primary">基于 Base64 下载图片</ElButton>
    <ElButton @click="handleDownloadOnlineFile" type="primary">根据文件地址下载文件</ElButton>
    <ElButton @click="handleDownload" type="primary">根据后台接口文件流下载</ElButton>
  </ElCard>
</template>
...
const handleDownloadOnlineImg = () => {
  saveAs('https://xiaoxian521.github.io/hyperlink/img/pure.png', 'test-d-online.png')
}

const handleDownloadBase64Img = () => {
  const base64Img =
  ...
  saveAs(base64Img, 'test-d-base64.png')
}

const handleDownloadOnlineFile = () => {
  saveAs('/test-files/test-zip.zip', 'test-d-online.zip')
}

const handleDownload = () => {
  appHttp
    .get('/test-files/test-mp4.mp4', {
      baseURL: '',
      responseType: 'blob'
    })
    .then(({ data }) => {
      saveAs(data, 'test-d-server-data.mp4')
    })
}
</script>
